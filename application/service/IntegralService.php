...
class IntegralService
{
        ...
        if(!empty($goods_all))
        {
            foreach($goods_all as $goods_id)
            {
                ...
                if(!empty($give_integral))
                {
                    ...
                    $user_re_integral = Db::name('User')->where(['id'=>$user['id']])->value('re_integral');

                    $user_integral = $user_integral + $user_re_integral;
     ...
     * 
     */
    public static function JifenHandle($params = [])
    {
        // 用户积分变动
        $user = Db::name('User')->where(['id'=>$params['order']['user_id']])->find();
        $user_integral = $user['integral'] + $user['re_integral'];
        //根据订单Id获取订单号
        $order = Db::name('Order')->where(['id'=>$params['order']['id']])->find();
        if(!Db::name('User')->where(['id'=>$params['order']['user_id']])->setDec('integral', $order['jifen_discount']))
        {
            return DataReturn('用户积分变动失败', -1);
        }
        // 积分日志
        IntegralService::UserIntegralLogAdd($params['order']['user_id'], $user_integral, $user_integral-$order['jifen_discount'], '购物抵扣积分--订单号：'.$order['order_no'], 0);
    }
     ...
     * 
     */
    public static function JifenFanli($params = [])
    {
        $user = Db::name('User')->where(['id'=>$params['order']['user_id']])->find();
        $user_integral = $user['integral'] + $user['re_integral'];
        $founds_one = MyC('common_fenxiao_yiji_num')==null ? 3 : MyC('common_fenxiao_yiji_num');
        $fan_one = $founds_one * 0.01 * $params['order']['total_price'];
        if(!Db::name('User')->where(['id'=>$params['order']['user_id']])->setInc('re_integral', $fan_one))
        {
            return DataReturn('用户积分变动失败', -1);
        }
        // 积分日志
        IntegralService::UserIntegralLogAdd($params['order']['user_id'], $user_integral, $user_integral+$fan_one, '购物返积分', 0);

        if($user['referrer'] != 0)
        {
            $user_one = Db::name('User')->where(['id'=>$user['referrer']])->find();

            $user_one_integral = $user_one['integral'] + $user_one['re_integral'];

            $founds_two = MyC('common_fenxiao_erji_num')==null ? 2 : MyC('common_fenxiao_erji_num');

            $fan_two = $founds_two * 0.01 * $params['order']['total_price'];

            if(!Db::name('User')->where(['id'=>$user['referrer']])->setInc('re_integral', $founds_two))
            {
                return DataReturn('用户积分变动失败', -1);
            }
            // 积分日志
            IntegralService::UserIntegralLogAdd($user['referrer'], $user_one_integral, $user_one_integral+$fan_two, '二级会员购物返积分', 0);

            if($user_one['referrer'] != 0)
            {
                $user_two = Db::name('User')->where(['id'=>$user_one['referrer']])->find();

                $user_two_integral = $user_two['integral'] + $user_two['re_integral'];

                $founds_three = MyC('common_fenxiao_sanji_num')==null ? 1 : MyC('common_fenxiao_sanji_num');

                $founds_three = $founds_three * 0.01 * $params['order']['total_price'];

                if(!Db::name('User')->where(['id'=>$user_one['referrer']])->setInc('re_integral', $founds_three))
                {
                    return DataReturn('用户积分变动失败', -1);
                }
                // 积分日志
                IntegralService::UserIntegralLogAdd($user_one['referrer'], $user_two_integral, $user_two_integral+$founds_three, '三级会员购物返积分', 0);
