...
class BuyService
{
    ...
    public static function CartList($params = [])
    {
        ...
        $field = 'c.*, g.title, g.images, g.inventory_unit, g.is_shelves, g.is_delete_time, g.buy_min_number, g.buy_max_number,g.is_jifen_discount';
     ...
     * 
     */
    public static function DiscountTotalPrice($params = [])
    {
        $data = $params;
        ...
        if(!empty($data))
        {
            ...
            {
                $v['total_price'] = $v['stock']*$v['price'];
                if($v['is_jifen_discount'])
                    $discount_total_price += $v['total_price'];
            }
        }
    ...
    public static function BuyGoods($params = [])
    {
        ...
        $p = [
            ...
            'field' => 'id, id AS goods_id, title, images, inventory_unit, buy_min_number, buy_max_number ,is_jifen_discount',
        ];
    ...
    public static function BuyGoodsCheck($params = [])
    {
        ...
        foreach($params['goods'] as $v)
        {
            ...
            if($v['is_jifen_discount'])
                $discount_total_price += $goods['price']*$v['stock'];
            // 总价
        ...
        $data = [
            ...
            'discount_price'    => $discount_total_price,
            'max_could_discount'=> $discount_total_price * MyC('common_jifen_max_discount_percent')
        ];
        return DataReturn('操作成功', 0, $data);
    }
    ...
    public static function OrderAdd($params = [])
    {
        ...
        $check = self::BuyGoodsCheck(['goods'=>$goods['data'],'jifen_discount'=>$params['jifen_discount']]);
        if(!isset($check['code']) || $check['code'] != 0)
        {
            return $check;
        }
        if($check['data']['max_could_discount'] < $params['jifen_discount'])
        {
            return $check;
        }

        if($params['jifen_discount'] > $params['user']['integral'])
        {
            return DataReturn('积分不足', -1);
        }
        // 用户地址
        ...
        $order = [
            ...
            'total_price'           => $check['data']['total_price']-$preferential_price-$params['jifen_discount'],
            'payment_id'            => isset($params['payment_id']) ? intval($params['payment_id']) : 0,
            'add_time'              => time(),
            'jifen_discount'        =>$params['jifen_discount'],
        ];
        if($order['status'] == 1)
        {
