...
class ZitiService
{
    ...
    {
        return empty($express_id) ? null : Db::name('Ziti_address')->where(['id'=>intval($express_id)])->value('name');
    }
    ...
    public static function ExpressList($params = [])
    {
        ...
        $data = Db::name('Ziti_address')->where($where)->field('id,icon,name,sort,is_enable')->order('sort asc')->select();
    ...
    public static function ExpressNodeSon($params = [])
    {
        ...
        $data = Db::name('Ziti_address')->field($field)->where(['pid'=>$id])->order('sort asc')->select();
        if(!empty($data))
        {
            $images_host = config('shopxo.images_host');
            foreach($data as &$v)
            {
                $v['is_son']            =   (Db::name('Ziti_address')->where(['pid'=>$v['id']])->count() > 0) ? 'ok' : 'no';
                $v['ajax_url']          =   MyUrl('admin/ziti/getnodeson', array('id'=>$v['id']));
                $v['delete_url']        =   MyUrl('admin/ziti/delete');
    ...
    public static function ExpressSave($params = [])
    {
        ...
        {
            $data['add_time'] = time();
            if(Db::name('Ziti_address')->insertGetId($data) > 0)
            {
            ...
            $data['upd_time'] = time();
            if(Db::name('Ziti_address')->where(['id'=>intval($params['id'])])->update($data))
            {
    ...
    public static function ExpressDelete($params = [])
    {
        ...
        // 开始删除
        if(Db::name('Ziti_address')->where(['id'=>intval($params['id'])])->delete())
        {
