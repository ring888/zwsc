...
class UserService
{
    ...
    public static function UserSave($params = [])
    {
        ...
        $data = [
            ...
            'realname'              => isset($params['user']['realname']) ? $params['user']['realname'] :  $params['realname'],
            'identity'              => isset($params['user']['identity']) ? $params['user']['identity'] :  $params['identity'],
    ...
    public static function Reg($params = [])
    {
        ...
        $p = [
            ...
            [
                'checked_type'      => 'fun',
                'key_name'          => 'referrer',
                'checked_data'      => 'CheckZzs',
                'error_msg'         => '推荐人ID格式不正确',
            ],
        ];
        $ret = ParamsChecked($params, $p);
        if($ret !== true)
        ...
        // 推荐人校验
        if(self::CheckRefId($params['referrer'],'id'))
        {
            return DataReturn('推荐人不存在', -4);
        }
     ...
     * 存在false,不存在true
     */
    private static function CheckRefId($ref, $field = 'id')
    {
        $id = Db::name('User')->where(array($field=>$ref))->find();
        return empty($id);
    }
    ...
    public static function PersonalSave($params = [])
    {
        ...
        $data = [
            ...
            'realname'      => empty($params['user']['realname']) ? $params['realname'] :  $params['user']['realname'],
            'identity'      => empty($params['user']['identity']) ? $params['identity'] :  $params['user']['identity'],
            'gender'        => intval($params['gender']),
            'upd_time'      => time(),
        ];
     ...
     * 
     */
    public static function UpdateUserJifen($params=[],$jifen_discount)
    {
        $params['integral'] -= $jifen_discount;
        return  Db::name('User')->update($params);
    }

    /**
     * 签到
     */
    public static function UserSign($params=[])
    {
        // 请求参数
        $p = [
            [
        ...
        $where[] = ['user_id', '=', $params['user']['id']];
        $check_sign = Db::name('Message')->where($where)->select();
        if(count($check_sign) >= 1)
        {
            return DataReturn('今天已经签过到，不可重复签到', -100);
        }

        // 开启事务
        Db::startTrans();

        $user = Db::name('User')->where(['id' => $params['user']['id']])->find();
        if($user['integral'] <= 0)
        {
            return DataReturn('无可用积分', -100);
        }
        $common_jifen_sign_balance_num = MyC('common_jifen_sign_balance_num')==null ? "10-20" : MyC('common_jifen_sign_balance_num');
        $common_jifen_sign_balance_num = explode('-',$common_jifen_sign_balance_num);

        //随机兑换数额
        $random = rand($common_jifen_sign_balance_num[0]==null?10:$common_jifen_sign_balance_num[0],$common_jifen_sign_balance_num[1]==null?20:$common_jifen_sign_balance_num[1]);
        //一个积分多少钱
        $price_jifen = MyC('common_jifen_equal_money')==null ? 0.1 : MyC('common_jifen_equal_money');
        //可用积分小于最小兑换数额时，全部兑换
        if($random > $user['integral'] && $user['integral'] > 0)
            $random = $user['integral'];
        //更新用户余额和积分
        $original_balance = $user['s_balance'];
        $original_jifen = $user['integral'];

        $user['s_balance'] += $random * $price_jifen;
        $user['integral'] -= $random;

        $user_update_status = Db::name('User')->update($user);
        //添加签到消息日志
        $message_status = MessageService::MessageAdd($user['id'], '签到', "签到随机兑换".$random."个积分到余额",2);
        //添加积分变动日志
        $jifen_status = IntegralService::UserIntegralLogAdd($user['id'], $original_jifen, $user['integral'], '签到兑换扣除', 0, $user['id']);
        //添加余额变动日志
        $balance_status = BalanceService::UserBalanceLogAdd($user['id'], $original_balance, $user['s_balance'], '签到兑换增加', 1, $user['id']);
        if($user_update_status && $message_status && $jifen_status && $balance_status)
        {
            // 提交事务
            Db::commit();
     ...
     * @param   [array]          $params [输入参数]
     */
    public static function WithdrawSave($params = [])
    {
        // 数据验证
        $p = [
            [
                'checked_type'      => 'fun',
                'checked_data'      => 'CheckNum',
                'key_name'          => 'money',
                'error_msg'         => '提现金额格式不正确',
            ],
            [
        ...
        if($params['money'] <= 0)
        {
            return DataReturn('非法金额', -10);
        }

        if($params['money'] > $params['user']['s_balance'])
        {
            return DataReturn('余额不足', -10);
        }
        // 开启事务
        Db::startTrans();
        //更新用户余额
        $user = $params['user'];
        $original_balance = $user['s_balance'];
        $user['s_balance'] -= $params['money'];
        $user_update_status = Db::name('User')->where(['id'=>$params['user']['id']])->update(['s_balance'=>$user['s_balance']]);
        //添加签到消息日志
        $message_status = MessageService::MessageAdd($user['id'], '提现', "余额提现".$params['money']."元",5);

        //添加余额变动日志
        $balance_status = BalanceService::UserBalanceLogAdd($user['id'], $original_balance, $user['s_balance'], '余额提现', 0, $user['id']);
        //添加提现记录
        $data = [
            'user_id'       => $params['user']['id'],
            'order_id'      => date('YmdHis',time()).GetNumberCode(),
            'money'         => $params['money'],
            'remarks'        => $params['remarks'],
            'add_time'      => time(),
        ];
        $withdraw_status = Db::name('Withdraw')->insertGetId($data);
        if($withdraw_status >0 && $message_status && $user_update_status && $balance_status)
        {
            // 提交事务
            Db::commit();
            // 更新用户session数据
