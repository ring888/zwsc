...
class OrderService
{
    ...
    private static function OrderPayHandle($params = [])
    {
        ...
        if(Db::name('Order')->where(['id'=>$params['order']['id']])->update($upd_data))
        {
            ...
            if(self::OrderHistoryAdd($params['order']['id'], 2, $params['order']['status'], '支付', 0, '系统'))
            {
                ...
                //积分返利
                IntegralService::JifenFanli($params);
				//根据订单ID扣除积分并写入记录
				$order = Db::name('Order')->where(['id'=>$params['order']['id']])->find();
				if($order['jifen_discount']>0)
					IntegralService::JifenHandle($params);
    ...
    public static function OrderDelivery($params = [])
    {
        ...
        $upd_data = [
            ...
            'ziti_address'      => isset($params['ziti_address']) ? $params['ziti_address'] : '',
            'delivery_time'     => time(),
            'upd_time'          => time(),
        ];
        if(Db::name('Order')->where($where)->update($upd_data))
